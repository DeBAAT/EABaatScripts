 ...
 * 
 * @author	J. de Baat, based on JavaScript-XML by Sparx Systems
 * @date	20-12-2024
 */
...
function XMLCreateXMLObject() /* : MSXML2.DOMDocument */
{
	...
	while (xmlDOM == null)
	{
		switch (attempt++)
		{
			...
			default: BLOGWarning("Could not create DOMDocument."); return;
		}
		try { xmlDOM = new COMObject( ProgId ); BLOGTrace("DOMDocument object created: " + ProgId); } catch (err) { xmlDOM = null; }
	}
...
function XMLParseXML( xmlDocument /* : String */ ) /* : MSXML2.DOMDocument */
{
	...
	if ( !parsed )
	{
		...
		BLOGWarning( _XMLDescribeParseError(xmlDOM.parseError) );
		xmlDOM = null;
	}
	...
	if ( node )
	{
		...
		if ( attributeMap != null )
		{
			...
			else
			{
				...
				BLOGWarning( "Node at path " + nodePath + 
		...
		else
		{
			...
			BLOGWarning( "Node at path " + nodePath + " does not contain attributes" );
		}
	}
	else
	{
		// Specified node not found
		BLOGWarning( "Node not found at path: " + nodePath );
	}
...
function XMLGetNamedAttribute( node /* : IXMLDOMNode */, attName /* : String */ ) /* : String */
{
	...
	{
		BLOGWarning( "Attribute not found: " + attName );
	}
